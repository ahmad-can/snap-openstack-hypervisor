...
def _configure_webdav_apache(snap: Snap) -> None:
    ...
    apache_etc_dir = snap.paths.snap / Path("etc/apache2")
    ...
    webdav_root = snap.paths.common / Path("var/webdav")
    webdav_root.mkdir(parents=True, exist_ok=True)
    webdav_root.chmod(0o757)

    dav_lock_dir = snap.paths.common / Path("var/webdav-lock")
    dav_lock_dir.mkdir(parents=True, exist_ok=True)
    dav_lock_dir.chmod(0o757)

    log_dir = snap.paths.common / Path("log/apache2")
    run_dir = snap.paths.common / Path("run/apache2")
    for path in (log_dir, run_dir):
        path.mkdir(parents=True, exist_ok=True)
        path.chmod(0o757)
